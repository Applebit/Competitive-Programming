...
class BearChairs {
    public:
	static vector<int> findPositions(vector<int> atLeast, int d) {
		vector<int> allocatedPositions;
		set< pair<int, int> > checkPositions;
		for (auto & x:atLeast)
		{
			for (auto & v:checkPositions)
				if (v.first <= x && x <= v.second) x = v.second + 1;
				allocatedPositions.push_back(x);
				checkPositions.insert({ x - d + 1, x + d - 1 });
		}
